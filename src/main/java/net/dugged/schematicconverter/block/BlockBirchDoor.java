...
public class BlockBirchDoor extends Block {
	...
	public Block getNewBlock() {
		Block block = new BlockBirchDoor().setProperty(facing.getName(), facing.getPropertyByIndex(getMetaBits(facing.getMetadataBitCount(), facing.getMetadataIndexBit())));
		...
		if (metadata == 8) {
			block = new BlockBirchDoor().setProperty(half.getName(), "upper").setProperty(hinge.getName(), "left");
		} else if (metadata == 9) {
			block = new BlockBirchDoor().setProperty(half.getName(), "upper").setProperty(hinge.getName(), "right");
		}
		return block;
	}
	...
	public Block getOldBlock() {
		Block block = new BlockBirchDoor().setMetadata(setMetaBits(facing.getCurrentIndex(), facing.getMetadataIndexBit()));
		...
		if (half.getValue().equals("upper")) {
			if (hinge.getValue().equals("right")) {
				block = new BlockBirchDoor().setMetadata(9);
			} else {
				block = new BlockBirchDoor().setMetadata(8);
			}
		}
		return block;
	}
